...
```
nom de la capture : `test-admin-user.png`
![alt text](test-admin-user.png)
...
# **Option: Use minikube's built-in kubeconfig**
# Get the token
TOKEN =$(kubectl create token dev-user -n dev --duration=24h)
# Use minikube's kubeconfig but with your token
kubectl --token=$TOKEN get pods -n dev

kubectl --token=$TOKEN get services -n dev
kubectl --token=$TOKEN get deployments -n dev
```

nom de la capture dev-user : `test-dev-user.png`
![alt text](test-dev-user.png)
...
# **Option: Use minikube's built-in kubeconfig**
# Get the token
TOKEN =$(kubectl create token viewer-user -n kube-system --duration=24h)
...
nom de la capture viewer-user : `test-viewer-user.png`
![alt text](test-viewer-user.png)
